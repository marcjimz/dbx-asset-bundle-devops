...
try:
    ...
    logger.warning(f"Could not create catalog/schema (may require admin privileges): {e}")
...
def engineer_features(df: DataFrame) -> DataFrame:
    ...
    features_df = df.withColumn(
        ...
        when(col("account_balance") > 0, col("income") / col("account_balance")).otherwise(0)
    ).withColumn(
        "transaction_frequency",
        when(col("days_since_last_transaction") > 0,
             col("num_transactions") / col("days_since_last_transaction")).otherwise(0)
    ).withColumn(
...
def validate_features(df: DataFrame) -> bool:
    ...
    # Validate ranges
    valid = (
        stats["min_age"] >= 18 and stats["max_age"] <= 120 and
        stats["min_credit"] >= 300 and stats["max_credit"] <= 850
    )
